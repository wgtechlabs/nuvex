...
export class RedisStorage implements StorageLayerInterface {
  private static readonly TRACKED_KEYS_SET = '__nuvex_keys__';
  ...
  async set(key: string, value: unknown, ttlSeconds?: number): Promise<void> {
    ...
    try {
      ...
      await this.client.sAdd(RedisStorage.TRACKED_KEYS_SET, key);
    } catch (error) {
  ...
  async delete(key: string): Promise<void> {
    ...
    try {
      ...
      await this.client.sRem(RedisStorage.TRACKED_KEYS_SET, key);
    } catch (error) {
  ...
  async clear(pattern = '*'): Promise<void> {
    ...
    try {
      ...
      if (pattern === '*') {
        await this.client.del(RedisStorage.TRACKED_KEYS_SET);
      } else if (keys.length > 0) {
        await this.client.sRem(RedisStorage.TRACKED_KEYS_SET, keys);
      }

      this.log('info', 'Redis L2: Cleared tracked Nuvex keys', {
        pattern,
        cleared: keys.length,
      });
    } catch (error) {
      this.log('error', 'Redis L2: Error clearing tracked keys', {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  }
  ...
  async keys(pattern = '*'): Promise<string[]> {
    ...
    try {
      const trackedKeys = await this.client.sMembers(
        RedisStorage.TRACKED_KEYS_SET,
      );
      ...
      this.log('error', 'Redis L2: Error listing tracked keys', {
        error: error instanceof Error ? error.message : String(error),
        pattern,
      });
      return [];
  ...
  async getTtl(key: string): Promise<number | null> {
    ...
    try {
      ...
      this.log('error', `Redis L2: Error getting TTL for key: ${key}`, {
        error: error instanceof Error ? error.message : String(error),
      });
      return null;
    }
