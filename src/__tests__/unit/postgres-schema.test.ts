...
describe('PostgreSQL Configurable Schema', () => {
  describe('generateNuvexSchemaSQL', () => {
    ...
    it('should reject invalid table names with SQL injection attempts', () => {
      ...
      expect(() => generateNuvexSchemaSQL(schema)).toThrow(
        'Invalid table name',
      );
    });
    ...
    it('should generate custom schema SQL for Telegram bot', () => {
      ...
      expect(sql).toContain(
        'CREATE INDEX IF NOT EXISTS idx_storage_cache_expires_at',
      );
      expect(sql).toContain(
        'CREATE OR REPLACE FUNCTION update_storage_cache_updated_at()',
      );
      expect(sql).toContain(
        'CREATE OR REPLACE FUNCTION cleanup_expired_storage_cache()',
      );
    });
  ...
  describe('PostgresStorage with custom schema', () => {
    ...
    it('should reject invalid column name in constructor', () => {
      ...
      expect(() => new PostgresStorage(config)).toThrow(
        'Invalid key column name',
      );
    });
  });
  ...
  describe('PostgresStorage readiness and auto setup', () => {
    const poolConstructor = (pg as unknown as { Pool: Mock<() => PoolType> })
      .Pool;

    beforeEach(() => {
      poolConstructor.mockImplementation(
        () => createMockPgPool() as unknown as PoolType,
      );
    });
    ...
    const createReadyPool = (
      ...
      const queryImpl = mock((sql: string, params?: unknown[]) => {
        ...
        if (
          options.failSelectWithMissingTable &&
          sql.startsWith('SELECT') &&
          sql.includes('expires_at')
        ) {
          return Promise.reject(
            Object.assign(
              new Error(`relation "${currentTableName}" does not exist`),
              { code: '42P01' },
            ),
          );
        }
    ...
    it('should auto setup schema and become ready on a fresh database', async () => {
      const pool = createReadyPool({ tableExists: false });
      poolConstructor.mockImplementation(() => pool as unknown as PoolType);
      ...
      await expect(
        storage.set('fresh:key', { ok: true }),
      ).resolves.toBeUndefined();
      expect(pool.query).toHaveBeenCalledWith(
        expect.stringContaining('CREATE TABLE IF NOT EXISTS nuvex_storage'),
      );
    });
    ...
    it('should honor custom schema when auto setup is enabled', async () => {
      ...
      expect(pool.query).toHaveBeenCalledWith(
        expect.stringContaining('CREATE TABLE IF NOT EXISTS storage_cache'),
      );
    });
