...
export class NuvexClient implements IStore {
  ...
  static getInstance(): NuvexClient {
    if (!NuvexClient.instance) {
      throw new Error(
        'Store not initialized. Call NuvexClient.initialize() first.',
      );
    }
    return NuvexClient.instance;
  }
  ...
  async clear(pattern?: string): Promise<number> {
    this.log(
      'warn',
      `Clearing storage${pattern ? ` with pattern: ${pattern}` : ' (all keys)'}`,
    );
  ...
  async cleanup(): Promise<{ cleaned: number; errors: number }> {
    ...
    try {
      ...
      this.log(
        'info',
        `Cleanup completed: ${cleaned} entries cleaned, ${errors} errors`,
      );
  ...
  async compact(): Promise<void> {
    ...
    try {
      ...
      this.log('error', 'Storage compaction failed', {
  ...
  async backup(
    destination?: string,
    options?: { incremental?: boolean; compression?: boolean },
  ): Promise<string> {
    ...
    try {
      ...
      if (incremental) {
        try {
          const lastBackupMetadata = await this.storage.get(
            '__nuvex_last_backup_metadata',
            {},
          );
          if (
            lastBackupMetadata &&
            typeof lastBackupMetadata === 'object' &&
            'timestamp' in lastBackupMetadata
          ) {
          ...
          this.log(
            'warn',
            'Could not retrieve last backup metadata, performing full backup',
            { error: (error as Error).message },
          );
        }
      }
      ...
      for (const key of allKeys) {
        ...
        const snapshotEntry = await this.storage.getBackupSnapshotEntry(key);

        if (snapshotEntry) {
          const itemData = {
            value: snapshotEntry.value,
            layerInfo: { layer: snapshotEntry.layer },
            ...(snapshotEntry.expiresAt
              ? { expiresAt: snapshotEntry.expiresAt }
              : {}),
          ...
          if (incremental && lastBackupTime) {
            ...
            const enhancedItemData = itemData as typeof itemData & {
              backupType?: string;
              lastBackupTime?: string;
            };
      ...
      if (fs && path) {
        ...
        if (compression) {
          try {
            ...
            this.log(
              'info',
              `Backup compressed and saved to ${backupFilePath}.gz`,
            );
          } catch (compressionError) {
            this.log('warn', 'Compression failed, saving uncompressed', {
        ...
        this.log(
          'warn',
          'File system not available, storing backup metadata internally (not recommended for production)',
        );
  ...
  async restore(
    source: string,
    options?: { clearExisting?: boolean; dryRun?: boolean },
  ): Promise<boolean> {
    ...
    try {
      ...
      if (fs && path) {
        ...
        const compressedExists = await fs.promises
        ...
        if (compressedExists) {
          try {
            ...
            const compressedData =
              await fs.promises.readFile(compressedFilePath);
            const decompressed = zlib.gunzipSync(compressedData);
            backupPackage = JSON.parse(decompressed.toString());
            this.log('info', 'Loaded compressed backup file');
          } catch (decompressionError) {
            this.log('error', 'Failed to decompress backup file', {
              error: (decompressionError as Error).message,
            });
            throw decompressionError;
          }
        } else {
          const uncompressedExists = await fs.promises
            .access(backupFilePath)
            .then(() => true)
            .catch(() => false);
          if (uncompressedExists) {
            const backupData = await fs.promises.readFile(
              backupFilePath,
              'utf8',
            );
      ...
      if (!backupPackage) {
        this.log(
          'warn',
          'External backup not found, checking internal storage',
        );
        const internalBackupMetadata = await this.storage.get(
          `__backup:${source}`,
          {},
        );
        ...
        this.log(
          'warn',
          'Internal backup found but data restoration from internal storage is limited',
        );
        return false;
      }
      ...
      for (const [key, itemData] of Object.entries(data)) {
        try {
          ...
          const { value, expiresAt, layerInfo } = item;
          ...
          if (expiresAt) {
            ...
            restoreOptions.ttl = remainingTtl;
          } else if (layerInfo?.ttl && layerInfo.ttl > 0) {
          ...
          errorCount++;
          this.log('error', `Error restoring key: ${key}`, {
